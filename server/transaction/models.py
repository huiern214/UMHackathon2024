...
class Transaction(models.Model):
    ...
    def get_transaaction(transactionID):
        result = supabase.table('transactions').select(
            '*').eq('transactionID', transactionID).execute()
        return result.data
...
class TransactionTable(models.Model):
    ...
    def get_transaction_tables_for_user(userID):
        result = supabase.table('transaction_tables').select(
            'transactionTableID', 'transactionTableName').eq('userID', userID).execute()
        return result.data
    ...
    def get_transaction_table(transactionTableID):
        result = supabase.table('transaction_tables').select(
            '*').eq('transactionTableID', transactionTableID).execute()
        return result.data
...
if __name__ == '__main__':
    print(TransactionTable.custom_query(
        "SELECT * FROM transaction_transactiontable WHERE transactionTableName = 'Test Table'"))
